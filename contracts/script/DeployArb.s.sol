...
contract DeployArb is Script {
    ...
    function run() external returns (WrappedUSDC token, SyntheticTokenBridge bridge) {
        ...
        token = new WrappedUSDC(predictedBridge);
        bridge = new SyntheticTokenBridge(token, BASE_SEPOLIA_USDC, BASE_SEPOLIA_CHAIN_ID);
        bridge.setRelayer(relayer);
        vm.stopBroadcast();

        if (address(bridge) != predictedBridge) revert UnexpectedBridgeAddress();
    }
}
