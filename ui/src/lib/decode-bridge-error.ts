...
function decodeBridgeCustomError({
  ...
  if (errorName === "InvalidBridgeTxInputs") {
    const [recipient, amount] = (args ?? []) as [
      Address | undefined,
      bigint | undefined,
    ];
    const evidence =
...
function decodePanic({
  ...
  const description =
    PANIC_MESSAGES[code] ??
    "An unexpected internal error occurred in the contract";
...
function decodeRevertString({
  ...
  const reason =
    errorName === "Error"
      ? ((reverted.data?.args as [string] | undefined)?.[0] ??
        reverted.reason ??
        "")
      : (reverted.reason ?? "");
 ...
 */
function decodeWalletRejection({
  error,
}: DecodeContext): DecodedBridgeError | undefined {
 ...
 */
function decodeEmptyRevertData({
  reverted,
}: DecodeContext): DecodedBridgeError | undefined {
  if (!reverted) return undefined;
...
function parseGasArgsBlock(
  ...
  const headingIndex = metaMessages.findIndex(
    (line) =>
      line === "Estimate Gas Arguments:" || line === "Raw Call Arguments:",
  );
...
function computeGasShortfall({
  ...
  const explicit =
    gasEstimate && gasEstimate.gas > 0n && gasEstimate.feePerGas > 0n
      ? gasEstimate
      : undefined;
...
function decodeInsufficientGas({
  ...
  const insufficientFunds = walkChain(error).find(
    (value): value is InsufficientFundsError =>
      value instanceof InsufficientFundsError,
  );
  ...
  const faucetClause = faucet
    ? ` — here's the faucet: ${faucet.faucetUrl}`
    : "";
...
function decodeUnknown({ reverted }: DecodeContext): DecodedBridgeError {
  const rawData = normaliseRawData(
    reverted?.raw ?? (reverted?.signature as Hex | undefined),
  );
  ...
  if (!rawData)
    return {
      kind: "unknown",
      message:
        "The transaction reverted, but no revert data was available to explain why.",
    };
