...
| A hosted relayer-side status/WebSocket backend for push notifications | Would feel "more real-time" than polling/on-chain event watching | **Explicitly out of scope per hard constraints** — no backend service. Also unnecessary: `BridgeInitiated`→`BridgeFinalized` on-chain events already fully determine the 3 coarse states | Live on-chain event subscription (`watchContractEvent` via WebSocket, HTTP-fallback) — already scoped |
